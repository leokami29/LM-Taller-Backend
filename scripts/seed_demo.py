...
def seed_demo(*, force: bool = False) -> None:
    ...
    try:
        ...
        eq1 = Equipment(
            ...
            first_received_date=datetime.utcnow().date() - timedelta(days=40),
        )
        eq2 = Equipment(
            company_id=company.id,
        ...
        inv_screen = InventoryItem(
            ...
            last_restocked_at=datetime.utcnow() - timedelta(days=5),
        )
        ...
        def mk_order(
            ...
            so = ServiceOrder(
                ...
                estimated_completion=datetime.utcnow() + timedelta(days=3) if status not in (OrderStatus.DELIVERED, OrderStatus.CANCELLED) else None,
                actual_completion=datetime.utcnow() - timedelta(days=1)
            ...
            if days_ago:
                so.created_at = datetime.utcnow() - timedelta(days=days_ago)
