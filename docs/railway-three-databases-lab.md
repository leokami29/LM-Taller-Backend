...
3. Insertar filas en `tenant_routing` (slug, `company_id`, `database_url`) coherentes con los seeds de cada tenant, o usar la API de plataforma `POST /api/platform/v1/companies` con `tenant_slug` y `tenant_database_url`.
