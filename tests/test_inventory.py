...
def test_inventory_used_in_repair_requires_service_order(client, db_session, seed_company_and_admin):
    company, _admin = seed_company_and_admin
    tech = User(
        company_id=company.id,
    ...
    res = client.post(
        ...
        headers={"Authorization": f"Bearer {token}"},
        json={
            "movement_type": "used_in_repair",
            "quantity_change": "-1",
        },
    )
    assert res.status_code == 400
    assert "orden" in res.json()["detail"].lower()
